...
echo "--------------------------------------------------"
sqlite3 movies_rating.db -box -echo "create view registration_statistics as select substr(u.register_date, 0, 5) as registered_at,count(u.id) as registrations_count from users u group by registered_at;"
...
echo "--------------------------------------------------"
sqlite3 movies_rating.db -box -echo "select m.title as movie, u1.name as first_user, u2.name as second_user from ratings r1 join ratings r2 on r1.movie_id = r2.movie_id and r1.id > r2.id join movies m on r1.movie_id = m.id join users u1 on r1.user_id = u1.id join users u2 on r2.user_id = u2.id order by m.id limit 200;"
echo " "
